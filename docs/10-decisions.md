...
## ADR-011：第一 UI 表面是独立工坊站

**决定**：先做独立浏览器工坊，而不是改 DSH 设置页或官方聊天 UI。卡片/快览/档案页的交互与视觉以 [11](11-ui-design-plan.md) 为准。

**理由**：市场控制平面本来就独立于 runtime；未连接 Companion 时也必须能逛目录。官方 Web 的 `--dsw-*` token 仅作为日后内嵌时的映射目标。

## ADR-012：工坊对齐官方产品工作台，不复制营销整页

**决定**：品牌核跟 `deepseek.com/harness` 的 `--ds-*`（蓝 `#4D6BFE`、Host Grotesk）；工具表面跟 `dsh web` 的 `--dsw-alias-*`。官网的社区插件、四模式、插件列表、轨迹海报在工坊里做成工具，见 [12](12-official-ui-alignment-plan.md)。

**理由**：官方「社区插件」只是 GitHub topic；四种模式和轨迹是展示。工坊的价值是把这些变成可检索、可比较、可挂载的控制面。

## 暂缓事项

- 精确前端技术栈（视觉与信息架构已见 11，实现栈未定）；
- 私有企业 registry；
- 付费和分成；
- 全量图数据库；
- 自动迁移 agent preset；
