...
const state = {
  ...
  media: { page: 1, q: '', status: '', submissionsPage: 1, submissionStatus: 'pending', reportsPage: 1, reportStatus: 'pending' },
...
function badge(status, text) {
  const labels = { approved: '已公开', pending: '待审核', hidden: '已隐藏', rejected: '已拒绝', accepted: '已接收', ready: '已缓存', fallback: '已降级', deferred: '已延后', active: '启用', disabled: '停用', completed: '已完成', partially_failed: '待继续', failed: '失败', queued: '排队中', discovering: '发现候选', verifying: '结构验证', verified: '已验证', open: '开放', locked: '已锁定', visible: '可见', resolved: '已处理', dismissed: '已驳回', thread: '讨论', reply: '回复', collection: '合集', review: '评价' };
...
async function renderOverview() {
  ...
  main.innerHTML = `
    <div class="stats">
      ...
      <article class="stat"><p>媒体异常</p><strong class="${overview.mediaAttention ? 'amber' : 'green'}">${overview.mediaAttention}</strong><small>${overview.mediaReady} 项已就绪</small></article>
      <article class="stat"><p>媒体反馈</p><strong class="${overview.pendingMediaReports ? 'amber' : 'green'}">${overview.pendingMediaReports}</strong></article>
      <article class="stat"><p>项目补录</p><strong class="${overview.pendingSubmissions ? 'amber' : 'green'}">${overview.pendingSubmissions}</strong></article>
...
async function renderMedia() {
  const mediaQuery = new URLSearchParams({ page: state.media.page, pageSize: 25 });
  if (state.media.q) mediaQuery.set('q', state.media.q); if (state.media.status) mediaQuery.set('status', state.media.status);
  const submissionQuery = new URLSearchParams({ page: state.media.submissionsPage, pageSize: 20 });
  if (state.media.submissionStatus) submissionQuery.set('status', state.media.submissionStatus);
  const reportQuery = new URLSearchParams({ page: state.media.reportsPage, pageSize: 20 });
  if (state.media.reportStatus) reportQuery.set('status', state.media.reportStatus);
  const [media, submissions, reports] = await Promise.all([
    api(`/admin/media?${mediaQuery}`), api(`/admin/plugin-submissions?${submissionQuery}`), api(`/admin/media-reports?${reportQuery}`),
  ]);
  const submissionPages = Math.max(1, Math.ceil(submissions.total / submissions.pageSize));
  const reportPages = Math.max(1, Math.ceil(reports.total / reports.pageSize));
  main.innerHTML = `<div class="section-note">封面由本站同源输出，远端媒体仅允许 GitHub Social Preview 或当前仓库固定 Commit 下的图片。“重试”会清理缓存并在下次请求时重新检查。</div>
    <div class="media-summary"><article><span>已缓存</span><strong>${media.summary.ready || 0}</strong></article><article><span>待检查</span><strong>${media.summary.pending || 0}</strong></article><article><span>已降级</span><strong>${media.summary.fallback || 0}</strong></article><article><span>失败</span><strong>${media.summary.failed || 0}</strong></article></div>
    <section class="panel media-panel"><div class="panel-head"><div><h2>媒体健康</h2><p>当前公开 Revision 的主封面状态</p></div></div><div class="panel-body"><form class="toolbar" id="mediaFilters"><input name="q" value="${escapeHtml(state.media.q)}" placeholder="搜索仓库或包名"><select name="status"><option value="">全部状态</option>${[['pending','待检查'],['ready','已缓存'],['fallback','已降级'],['failed','失败']].map(([value,label]) => `<option value="${value}" ${state.media.status === value ? 'selected' : ''}>${label}</option>`).join('')}</select><button class="button primary">筛选</button></form><div class="table-wrap"><table><thead><tr><th>封面 / 插件</th><th>来源</th><th>状态</th><th>最近检查</th><th>操作</th></tr></thead><tbody>${media.items.map(item => `<tr><td><div class="media-plugin"><img src="/api/v1/plugins/${encodeURIComponent(item.pluginId)}/cover.svg" alt=""><div><strong>${escapeHtml(item.name)}</strong><small>${escapeHtml(item.fullName)}</small></div></div></td><td><small>${escapeHtml(item.sourceType)}<br><code title="${escapeHtml(item.sourceUrl)}">${escapeHtml(item.sourceUrl)}</code></small></td><td>${badge(item.status)}${item.error ? `<small class="reason">${escapeHtml(item.error)}</small>` : ''}</td><td><small>${item.checkedAt ? formatTime(item.checkedAt) : '尚未请求'}${item.bytes ? `<br>${Math.ceil(item.bytes / 1024)} KiB` : ''}</small></td><td><button data-media-retry="${escapeHtml(item.pluginId)}">清理并重试</button></td></tr>`).join('') || '<tr><td colspan="5" class="empty">当前筛选下没有媒体记录</td></tr>'}</tbody></table></div>${pagination(media, 'media')}</div></section>
    <section class="panel media-panel"><div class="panel-head"><div><h2>GitHub 项目补录</h2><p>接收后仍需由 Topic 同步、结构验证和插件审核才能公开</p></div><select id="submissionStatus"><option value="pending" ${state.media.submissionStatus === 'pending' ? 'selected' : ''}>待审核</option><option value="accepted" ${state.media.submissionStatus === 'accepted' ? 'selected' : ''}>已接收</option><option value="rejected" ${state.media.submissionStatus === 'rejected' ? 'selected' : ''}>已拒绝</option><option value="" ${state.media.submissionStatus === '' ? 'selected' : ''}>全部</option></select></div><div class="panel-body"><div class="table-wrap"><table><thead><tr><th>仓库</th><th>提交用户</th><th>状态</th><th>备注</th><th>操作</th></tr></thead><tbody>${submissions.items.map(item => `<tr><td><a class="row-title" href="${escapeHtml(item.repositoryUrl)}" target="_blank" rel="noopener">${escapeHtml(item.repositoryFullName)}</a><small>${formatTime(item.createdAt)}</small></td><td>${escapeHtml(item.username)}</td><td>${badge(item.status)}</td><td><small>${escapeHtml(item.note || '—')}</small></td><td><div class="actions">${item.status !== 'accepted' ? `<button data-submission-id="${escapeHtml(item.id)}" data-submission-status="accepted">接收</button>` : ''}${item.status !== 'rejected' ? `<button class="danger" data-submission-id="${escapeHtml(item.id)}" data-submission-status="rejected">拒绝</button>` : ''}</div></td></tr>`).join('') || '<tr><td colspan="5" class="empty">当前筛选下没有补录记录</td></tr>'}</tbody></table></div><div class="pagination"><span>第 ${submissions.page} / ${submissionPages} 页 · 共 ${submissions.total} 项</span><button data-submission-page="${submissions.page - 1}" ${submissions.page <= 1 ? 'disabled' : ''}>‹</button><button data-submission-page="${submissions.page + 1}" ${submissions.page >= submissionPages ? 'disabled' : ''}>›</button></div></div></section>
    <section class="panel media-panel"><div class="panel-head"><div><h2>媒体问题反馈</h2><p>用户针对封面、截图或显示异常提交的说明</p></div><select id="mediaReportStatus"><option value="pending" ${state.media.reportStatus === 'pending' ? 'selected' : ''}>待处理</option><option value="resolved" ${state.media.reportStatus === 'resolved' ? 'selected' : ''}>已处理</option><option value="dismissed" ${state.media.reportStatus === 'dismissed' ? 'selected' : ''}>已驳回</option><option value="" ${state.media.reportStatus === '' ? 'selected' : ''}>全部</option></select></div><div class="panel-body"><div class="report-grid">${reports.items.map(item => `<article class="report-card"><div class="report-card-head">${badge(item.status)}<time>${formatTime(item.createdAt)}</time></div><h3>${escapeHtml(item.pluginName)}</h3><p>${escapeHtml(item.reason)}</p><dl><div><dt>反馈用户</dt><dd>${escapeHtml(item.reporterName)}</dd></div>${item.resolution ? `<div><dt>处理说明</dt><dd>${escapeHtml(item.resolution)}</dd></div>` : ''}</dl>${item.status === 'pending' ? `<div class="actions"><button data-media-report="${escapeHtml(item.id)}" data-report-status="resolved">标记已处理</button><button data-media-report="${escapeHtml(item.id)}" data-report-status="dismissed">驳回</button></div>` : ''}</article>`).join('') || '<div class="empty">当前筛选下没有媒体反馈</div>'}</div><div class="pagination"><span>第 ${reports.page} / ${reportPages} 页 · 共 ${reports.total} 项</span><button data-media-report-page="${reports.page - 1}" ${reports.page <= 1 ? 'disabled' : ''}>‹</button><button data-media-report-page="${reports.page + 1}" ${reports.page >= reportPages ? 'disabled' : ''}>›</button></div></div></section>`;
  document.getElementById('mediaFilters').addEventListener('submit', event => { event.preventDefault(); const data = new FormData(event.currentTarget); state.media = { ...state.media, page: 1, q: String(data.get('q') || '').trim(), status: String(data.get('status') || '') }; renderMedia().catch(showFailure); });
  document.getElementById('submissionStatus').addEventListener('change', event => { state.media.submissionStatus = event.currentTarget.value; state.media.submissionsPage = 1; renderMedia().catch(showFailure); });
  document.getElementById('mediaReportStatus').addEventListener('change', event => { state.media.reportStatus = event.currentTarget.value; state.media.reportsPage = 1; renderMedia().catch(showFailure); });
  main.querySelectorAll('[data-media-retry]').forEach(button => button.addEventListener('click', async () => { try { await api(`/admin/plugins/${encodeURIComponent(button.dataset.mediaRetry)}/media/retry`, { method: 'POST' }); toast('媒体缓存已清理，下次访问将重新检查'); await renderMedia(); } catch (error) { toast(error.message, 'error'); } }));
  main.querySelectorAll('[data-submission-id]').forEach(button => button.addEventListener('click', async () => { const rejected = button.dataset.submissionStatus === 'rejected'; const note = await confirmAction({ title: rejected ? '拒绝该项目补录？' : '接收该项目补录？', body: rejected ? '请填写可追溯的拒绝原因。' : '接收只表示进入同步候选，不会绕过结构验证和公开审核。', reason: rejected, confirmText: rejected ? '拒绝' : '接收' }); if (note === null) return; try { await api(`/admin/plugin-submissions/${encodeURIComponent(button.dataset.submissionId)}`, { method: 'PATCH', body: JSON.stringify({ status: button.dataset.submissionStatus, ...(note ? { note } : {}) }) }); toast('补录审核状态已更新'); await renderMedia(); } catch (error) { toast(error.message, 'error'); } }));
  main.querySelectorAll('[data-media-report]').forEach(button => button.addEventListener('click', async () => { const resolution = await confirmAction({ title: button.dataset.reportStatus === 'resolved' ? '确认已处理该媒体问题？' : '确认驳回该媒体反馈？', body: '请填写可追溯的处理说明。', reason: true }); if (resolution === null) return; try { await api(`/admin/media-reports/${encodeURIComponent(button.dataset.mediaReport)}`, { method: 'PATCH', body: JSON.stringify({ status: button.dataset.reportStatus, resolution }) }); toast('媒体反馈已更新'); await renderMedia(); } catch (error) { toast(error.message, 'error'); } }));
  main.querySelectorAll('[data-submission-page]').forEach(button => button.addEventListener('click', () => { state.media.submissionsPage = Number(button.dataset.submissionPage); renderMedia().catch(showFailure); }));
  main.querySelectorAll('[data-media-report-page]').forEach(button => button.addEventListener('click', () => { state.media.reportsPage = Number(button.dataset.mediaReportPage); renderMedia().catch(showFailure); }));
  bindCommon();
}
